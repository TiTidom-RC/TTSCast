...
class ttscast extends eqLogic
{
    ...
    */
    ...
    public static function playTTS($gHome=null, $message=null, $options=null) {
        ...
        $ttsSpeed = config::byKey('gCloudTTSSpeed', 'ttscast', '1.0');
        $ttsOptions = $options;
        $value = array('cmd' => 'playtts', 'ttsLang' => $ttsLang, 'ttsEngine' => $ttsEngine, 'ttsSpeed' => $ttsSpeed, 'ttsOptions' => $ttsOptions, 'ttsText' => $ttsText, 'ttsGoogleUUID' => $ttsGoogleUUID, 'ttsVoiceName' => $ttsVoiceName, 'ttsRSSVoiceName' => $ttsRSSVoiceName, 'ttsRSSSpeed' => $ttsRSSSpeed);
        self::sendToDaemon($value);
    }
   ...
   */
    ...
    public function postSave() {
        ...
        if (!is_object($cmd)) {
	        ...
	        $cmd->setIsVisible(1);
            $cmd->setDisplay('parameters', array("title_placeholder" => "Options : [volume]", "message_placeholder" => "TTS"));
            $cmd->setOrder($orderCmd++);
            $cmd->save();
        } else {
            $orderCmd++;
        ...
        if ($this->getConfiguration('cast_type', '') == 'cast') {
            ...
            if (!is_object($cmd)) {
                ...
                $cmd->setSubType('message');
                $cmd->setIsVisible(1);
                $cmd->setDisplay('parameters', array("title_placeholder" => "Options : [volume, playlist, enqueue]", "message_placeholder" => "Vidéo Id"));
                $cmd->setOrder($orderCmd++);
                $cmd->save();
            } else {
                $orderCmd++;
            ...
            if (!is_object($cmd)) {
                ...
                $cmd->setSubType('message');
                $cmd->setIsVisible(1);
                $cmd->setDisplay('parameters', array("title_placeholder" => "Options : [force, quit_app, reload_seconds]", "message_placeholder" => "Page Web (URL)"));
                $cmd->setOrder($orderCmd++);
                $cmd->save();
            } else {
                $orderCmd++;
