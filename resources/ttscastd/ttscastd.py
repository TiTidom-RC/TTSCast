...
def castToGoogleHome(urltoplay, googleName):
    if googleName != '':
        ...
        logging.debug('[DAEMON][Cast] Diffusion lancée :: %s', json.dumps(cast.media_controller.status))
        
        while cast.media_controller.status.player_state == 'PLAYING':
            time.sleep(1)
            logging.debug('[DAEMON][Cast] Diffusion en cours :: %s', json.dumps(cast.media_controller.status))
